...
def RockNetA(n_classes, in_chans=22, in_samples=1125, n_windows=5, attention='mha',
    ...
    for i in range(n_windows):
        ...
        #Importo Reshape per il secondo TCN Block
        from tensorflow.keras.layers import Reshape
        # Aggiungi un secondo TCN Block
        block3 = Reshape((-1, tcn_filters))(block3)
        block4 = TCN_block_(input_layer=block3, input_dimension=tcn_filters, depth=tcn_depth,
                            kernel_size=tcn_kernelSize, filters=tcn_filters,
                            weightDecay=conv_weightDecay, maxNorm=conv_maxNorm,
                            dropout=tcn_dropout, activation=tcn_activation)
        block4 = Lambda(lambda x: x[:, -1, :])(block4)

        if fuse == 'average':
            sw_concat.append(Dense(n_classes, kernel_regularizer=L2(dense_weightDecay))(block4))
        elif fuse == 'concat':
            if i == 0:
                sw_concat = block4
            else:
                sw_concat = Concatenate()([sw_concat, block4])
...
def TCN_block_(input_layer,input_dimension,depth,kernel_size,filters, dropout,
    ...
    block = Activation(activation)(block)
    block = Dropout(dropout)(block)
    #Aggiungo un altro blocco di convoluzione
    block = Conv1D(filters,kernel_size=kernel_size,dilation_rate=1,activation='linear',
                    kernel_regularizer=L2(weightDecay),
                    kernel_constraint = max_norm(maxNorm, axis=[0,1]),
                    padding = 'causal',kernel_initializer='he_uniform')(block)
