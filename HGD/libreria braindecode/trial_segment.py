...
def create_signal_target_from_raw_mne(
    ...
    print("RAW: ",raw.info)
    print("ANNOTAIONS:",raw.annotations)
    
    stim_channel_data = raw.get_data(picks='stim')  # Prende i dati dal canale di Stimulus
    print("Valori unici nel canale di Stimulus:", np.unique(stim_channel_data))
    stim_channel_data = raw.get_data(picks='stim')  # Prende i dati dal canale di Stimulus
    
    stim_channel_data = raw.get_data(picks='stim')  # Prende i dati dal canale di Stimulus

    # Trova i valori unici nel canale di stimolo
    unique_values = np.unique(stim_channel_data)

    # Stampa i valori unici    
    x = 1
    temp = x / raw.info['sfreq']
    print("Tempo di un campione:", temp)
    
    events = mne.find_events(raw,min_duration=0.002)
    event_durations = np.diff(events[:, 0])
    print("Numero di eventi:", len(events))
    print("Durate degli eventi:", event_durations)
    print("Frequenza di campionamento:", raw.info['sfreq'])
   
    events = np.array([events[:, 0], events[:, 2]]).T
...
def create_signal_target(data, events, fs, name_to_start_codes, epoch_ival_ms,
    ...
    """
    print("DEBUG--CREATE_SIGNAL_TARGET")
    print("Dati in create_signal_target:", data.shape)
    print("Eventi: ",len(events))
    print("Codici di fine:", name_to_stop_codes)
...
def _create_cnt_y_and_trial_bounds_from_start_and_ival(
    ...
    # Converti l'intervallo di tempo in campioni
    ival_in_samples = ms_to_samples(np.array(epoch_ival_ms), fs)
    print("Intervallo in campioni:", ival_in_samples)
    
    start_offset = np.int32(np.round(ival_in_samples[0]))
    print("Offset di inizio:", start_offset)
    
    stop_offset = np.int32(np.ceil(ival_in_samples[1]))
    print("Offset di fine:", stop_offset)
    
    # Mapping da codice marker a nome e y
    ...
    # Inizializza il contatore dei trials
    ...
    event_codes = [event[1] for event in events]  # terza colonna (indice 2)
    
    print(f"Tipi di dati in events[:, 0]: {set(event_types)}")
    print(f"Tipi di dati in events[:, 1]: {set(event_codes)}")
    events = np.asarray(events)
    
    # DEBUG: Ciclo attraverso gli eventi
    for i_sample, mrk_code in zip(events[:, 0], events[:, 1]):
        #print(f"\nElaborando evento: {mrk_code} (indice: {i_sample})")
        
        start_sample = int(i_sample) + start_offset
        stop_sample = int(i_sample) + stop_offset
        
        # DEBUG: Verifica start e stop
        #print(f"start_sample: {start_sample}, stop_sample: {stop_sample}")
        
        if mrk_code in mrk_code_to_name_and_y:
            # Debug: Controlliamo se gli indici sono validi
            if start_sample < 0:
                log.warning(
                    "Ignora trial con codice marker {:d}, partirebbe dal "
                    "campione {:d}".format(mrk_code, start_sample))
                continue
            if stop_sample > n_samples:
                log.warning("Ignora trial con codice marker {:d}, finirebbe a "
                            "campione {:d} di {:d}".format(
                    mrk_code, stop_sample - 1, n_samples - 1))
                continue

            name, this_y = mrk_code_to_name_and_y[mrk_code]
            #print(f"Nome: {name}, y: {this_y}")
            
            # Salva gli indici degli intervalli
            i_start_stops.append((start_sample, stop_sample))
            
            # DEBUG: Controlliamo l'aggiornamento di cnt_y
            #print(f"Contenuto di cnt_y[{start_sample}:{stop_sample}, {this_y}] prima dell'aggiornamento:")
            #print(cnt_y[start_sample:stop_sample, this_y])
            
            # Aggiornamento di cnt_y
            cnt_y[start_sample:stop_sample, this_y] = 1
            
            # DEBUG: Verifica l'aggiornamento
            #print(f"Contenuto di cnt_y[{start_sample}:{stop_sample}, {this_y}] dopo l'aggiornamento:")
            #print(cnt_y[start_sample:stop_sample, this_y])
            
            # Incrementa il conteggio dei trials per classe
            class_to_n_trials[name] += 1
            #print(f"Updated cnt_y[{start_sample}:{stop_sample}, {this_y}]:", cnt_y[start_sample:stop_sample, this_y])
    
    # DEBUG: Stampa l'array cnt_y finale
    #Stampo i valori di cnt_y che sono stati aggiornati poichè corrispondono a trials validi da 5001 a 7001
    print("cnt_y (ultimi 5000 valori):", cnt_y[5000:])
    print("Dimensione di cnt_y:", cnt_y.shape)
    
    # Informazioni finali sul numero di trials per classe
...
def _create_cnt_y_and_trial_bounds_from_start_stop(
    ...
    """
    print(">>> Entrato in _create_cnt_y_and_trial_bounds_from_start_stop")
    ...
    print("-----Start code to name and y:", start_code_to_name_and_y)
    ...
    log.info("Trial per class:\n{:s}".format(str(class_to_n_trials)))
    print("cnt_y_SHape:", cnt_y.shape)
    print("CNT_Y--5001",cnt_y[5000: ])
    print("i_start_stops shape:", len(i_start_stops))
    return cnt_y, i_start_stops
