...
class BBCIDataset(object):
    ...
    def get_all_sensors(filename, pattern=None):
        ...
        with h5py.File(filename, 'r') as h5file:
            ...
            all_sensor_names = [''.join(chr(c) for c in h5file[obj_ref][:].squeeze()) for
                                obj_ref in clab_set]
    ...
    def _add_markers(self, cnt):
        with h5py.File(self.filename, 'r') as h5file:
            ...
            all_class_names = [''.join(chr(c) for c in h5file[obj_ref][:].squeeze())
        ...
        event_arr = np.array([event_times_in_samples,
                     [0] * len(event_times_in_samples),
                     event_classes])
        # Aggiungi gli eventi usando il metodo appropriato
        cnt.add_events(event_arr.T)  # Impostare `overwrite=True` se gli eventi esistenti devono essere sovrascritti
        return cnt
